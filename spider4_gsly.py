...
def checkHomePage(homePage):
    ...
    for a in a_list:
        queue.append(url_join(homePage, a['href']))
    print(queue)
    #fp = open(str(time.strftime('%Y-%m-%d')) + homePage + '首页.txt', 'w')
    fp = open(homePage.split('http://')[1].split('/')[0] + '首页' + str(time.strftime('%Y-%m-%d')) + '.txt', 'w')
...
def checkDeadURL(rootURl):
    ...
    fp = open(homePage.split('http://')[1].split('/')[0] + '全站死链' + str(time.strftime('%Y-%m-%d')) + '.txt', 'w')
    while queue:
        try:
            ...
            if (not res.status_code == 200) or (len(res.text) < 1500):
                ...
                fp.writelines(' 页面读取错误, 错误代码{}, 标题是 : {} ,url = {} ,父url = {} ################## \n'.format(error['code'], error['title'], error['url']), error['parent_url'])
                fp.flush()
                errors.append(error)
                continue
            res.encoding = 'gbk'
            ...
            fp.writelines('抓取错误!  | 标题 : {} , url : {} , 父目录 : {}  ----------------\n'.format(count,  url_title, url, parent_url))
            fp.flush()
            continue
...
if __name__ == '__main__':
    homePage = 'http://www.gsfao.gov.cn/'
